...
public class SystemControllers {
    ...
    @GetMapping(path = "/statusCompany", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> getStatusCompany() {
        try {
    ...
    @GetMapping(path = "/statusCompany/{id}", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> getStatusCompanyById(@PathVariable Long id) {
        try {
            return systemService.getDetailStatusCompany(id);
        } catch (Exception e){
            return handleException(e);
        }
    }
