...
public class SystemControllers {
    ...
    //Perbaikan pada inspection
    @GetMapping(path = "/inspectionQuestion/{areaKerjaId}", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> getQuestions(@PathVariable Long areaKerjaId) {
        try {
            return systemService.getInspectionQuestion(areaKerjaId);
        } catch (Exception e) {
            return handleException(e);
        }
    }
    ...
    @PostMapping(path = "/inspection_answer/add", consumes = MediaType.APPLICATION_JSON_VALUE)
    ...
    //
    @PostMapping(path = "/detail_daily_inspection", produces = MediaType.APPLICATION_JSON_VALUE)
